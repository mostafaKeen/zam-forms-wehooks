<?php
/**
 * Dubai Property Guide Download Webhook Implementation
 * 
 * Captures data, stores it in dubai_property_guide_download.json,
...
file_put_contents(__DIR__ . '/dubai_property_guide_download_requests.log', date('[Y-m-d H:i:s] ') . "Raw: " . $rawData . " | POST: " . json_encode($_POST) . PHP_EOL, FILE_APPEND);
...
if ($data) {
    // Local Logging (Store in dubai_property_guide_download.json)
    $logEntry = json_encode([
        'timestamp' => date('Y-m-d H:i:s'),
        'data' => $data ?: $rawData
    ]) . PHP_EOL;

    file_put_contents(__DIR__ . '/dubai_property_guide_download.json', $logEntry, FILE_APPEND);
    ...
    $name = $formFields['name']['value'] ?? 'Dubai Property Guide Lead';
    ...
    $bitrixFields = [
        'fields' => [
            'TITLE' => 'Dubai Property Guide Download: ' . $name,
            'NAME' => $name,
            'SOURCE_ID' => 'CALLBACK',            // "Zamprime Website"
            'ASSIGNED_BY_ID' => 1,                // Default ID (RE/MAX ZAM)
            'UF_CRM_1773862147183' => '21080',    // Subsource: "Dubai Property Guide Download"
    ...
    file_put_contents(__DIR__ . '/dubai_property_guide_download_forward_log.txt', $logStatus . PHP_EOL, FILE_APPEND);
}
