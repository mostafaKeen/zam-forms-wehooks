<?php
/**
 * Tenant Guide Download Webhook Implementation
 * 
 * Captures data, stores it in tenant_guide_download.json,
...
// 2. Local Logging (Store in tenant_guide_download.json)
$logEntry = json_encode([
    'timestamp' => date('Y-m-d H:i:s'),
    'data' => $data ?: $rawData
]) . PHP_EOL;

file_put_contents(__DIR__ . '/tenant_guide_download.json', $logEntry, FILE_APPEND);
...
$name = $formFields['name']['value'] ?? 'Tenant Guide Lead';
...
$bitrixFields = [
    'fields' => [
        'TITLE' => 'Tenant Guide Download: ' . $name,
        'NAME' => $name,
        'SOURCE_ID' => 'CALLBACK',            // "Zamprime Website"
        'ASSIGNED_BY_ID' => 1,                // Default ID (RE/MAX ZAM)
        'UF_CRM_1773862147183' => '21078',    // Subsource: "Tenant Guide Download"
...
file_put_contents(__DIR__ . '/tenant_guide_download_forward_log.txt', $logStatus . PHP_EOL, FILE_APPEND);
