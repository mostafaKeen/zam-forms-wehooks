<?php
/**
 * Legal Guide Download Webhook Implementation
 * 
 * Captures data, stores it in legal_guide_download.json,
...
// 2. Local Logging (Store in legal_guide_download.json)
$logEntry = json_encode([
    'timestamp' => date('Y-m-d H:i:s'),
    'data' => $data ?: $rawData
]) . PHP_EOL;

file_put_contents(__DIR__ . '/legal_guide_download.json', $logEntry, FILE_APPEND);
...
$name = $formFields['name']['value'] ?? 'Legal Guide Lead';
...
$bitrixFields = [
    'fields' => [
        'TITLE' => 'Legal Guide Download: ' . $name,
        'NAME' => $name,
        'SOURCE_ID' => 'CALLBACK',            // "Zamprime Website"
        'ASSIGNED_BY_ID' => 1,                // Default ID (RE/MAX ZAM)
        'UF_CRM_1773862147183' => '21074',    // Subsource: "Legal Guide Download"
...
file_put_contents(__DIR__ . '/legal_guide_download_forward_log.txt', $logStatus . PHP_EOL, FILE_APPEND);
